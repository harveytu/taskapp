...
const TaskApp = {
  ...
  async switchTaskList(taskListId) {
    try {
      ...
      // Save current list ID to localStorage for sync
      localStorage.setItem('task_current_list_id', taskListId);
      await this.loadTasks();
      this.updateTaskListDropdown();
      // Sync to native for widget
      if (window.DataSyncBridge && window.DataSyncBridge.syncTasksToNative) {
        await window.DataSyncBridge.syncTasksToNative();
      }
    } catch (error) {
